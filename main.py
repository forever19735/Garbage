...
def handle_message(event):
    ...
    if hasattr(event.message, 'text') and event.message.text.strip().startswith('@'):
        unknown_cmd = event.message.text.strip().split()[0]
        suggestions = suggest_commands(unknown_cmd)
        error_msg = ERROR_TEMPLATES['unknown_command'].format(
            command=unknown_cmd,
